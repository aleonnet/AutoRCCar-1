__author__ = 'zhengwang mod by alessandrobarbosa'
...
class RCControl(object):
    ...
    def __init__(self):
        ...
        self.ws = websocket.WebSocketApp("ws://192.168.2.49:1880/ws/vr",
    ...
    def on_message(self, ws, message):
        ...
        if payload['topic']=="manual":
            print payload['movement'] + payload ['direction']
    ...
    def sendit(self, movement, direction):
        if movement=="Forward":
            dy1 = "-40"
        if movement=="Reverse":
            dy1 = "40"
        ...
        if direction == "Right":
            ...
            dx1 = "0"
        topic = "computer"
        msg = '{"topic":"' + topic + '","dx1":"' +  dx1 + '","dy1":"' +  dy1 + '"}'
        self.ws.send(msg)

    def steer(self, prediction):
        if prediction == 2:
            self.sendit("Forward", "Straight")
            print("Forward")
        elif prediction == 0:
            self.sendit("Forward", "Left")
            print("Left")
        elif prediction == 1:
            self.sendit("Forward", "Right")
            print("Right")
        else:
            self.stop()

    def stop(self):
        self.sendit("Stop", "Straight")
...
#    distance_thread = threading.Thread(target=server_thread2, args=('192.168.2.153', 8002))
#    distance_thread.start()
#    video_thread = threading.Thread(target=server_thread('192.168.2.153', 8000))
#    video_thread.start()
